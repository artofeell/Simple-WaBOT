...
const handler = async (sock, m, options) => {
    ...
    const isCommand = message.body.startsWith(config.prefix);
    let args = [];
    let command = '';
    let plugin = null;
    ...
    if (plugin) {
        ...
        // Filter agar tidak membalas media, stiker, dsb jika perlu
        // Contoh: hanya balas pesan teks di chat pribadi
        if (!message.isGroup) {
            const aiPlugin = require('./plugins/AI/ustadz.js');
            await aiPlugin.run(sock, message, message.body.split(' '));
        }
    }
};
